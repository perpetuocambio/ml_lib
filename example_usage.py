...
class LinearRegressionEstimator(EstimatorInterface[np.ndarray, np.ndarray]):
    ...
    def _fit_impl_raw(self, X: np.ndarray, y: np.ndarray, **kwargs) -> 'LinearRegressionEstimator':
        ...
        self.logger.info(f"Modelo ajustado con éxito: weights shape = {self.weights.shape}")
        
        return self
    
    def fit(self, X: np.ndarray, y: np.ndarray, **kwargs) -> 'LinearRegressionEstimator':
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if X.shape[1] != self.weights.shape[0]:
            raise ValueError(f"La dimensionalidad de entrada {X.shape[1]} no coincide con la dimensionalidad del modelo {self.weights.shape[0]}")
...
def main():
    ...
    true_bias = 1.0
    y = X @ true_weights + true_bias + 0.1 * np.random.randn(100)  # Añadir algo de ruido
