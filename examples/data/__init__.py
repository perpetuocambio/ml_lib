...
class EcologicalDataReader:
    ...
    def read_enviromental_data_csv(
        file_path: Union[str, Path], site_column: str = "site"
    ...
    def inverse_transform(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.method == "relative" and self.fitted:
            ...
            raise NotImplementedError(
                f"Inversa no implementada para el método: {self.method}"
            )
...
class DataFilter:
    ...
    def filter_by_occurrence(
        ...
        # Aplicar filtros
        valid_items = (occurrence_proportion >= min_occurrence_proportion) & (
            total_values >= min_total_value
        )

        filtered_matrix = data_matrix[valid_items]
        filtered_names = [
            item_names[i] for i, is_valid in enumerate(valid_items) if is_valid
        ]
