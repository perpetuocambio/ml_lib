...
class SQLiteAgentRepository:
    ...
    async def save(self, agent_data: AgentData) -> None:
    ...
    async def get_by_id(self, agent_id: str) -> AgentData | None:
        """Get agent by ID."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
    ...
    async def get_by_project_id(self, project_id: str) -> list[AgentData]:
    ...
    async def get_by_user_id(self, user_id: str) -> list[AgentData]:
