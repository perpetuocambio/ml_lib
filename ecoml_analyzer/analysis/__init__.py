...
class EcologicalAnalyzer:
    ...
    def __init__(self):
        ...
        self.scaler = StandardScaler()  # Este sería reemplazado por un componente de ml_lib
        self.pca_components = None
        self.clusters = None
        
        self.logger.info("EcologicalAnalyzer inicializado - usando componentes generales de ml_lib")
    ...
    def analyze_composition(
        ...
        self.validation_service.validate_input_shape(data_matrix, 2, "analyze_composition")
        
        # Cálculos generales aplicables a datos multidimensionales
        total_values = np.sum(data_matrix, axis=0)  # Suma por característica
        relative_values = data_matrix / (np.sum(data_matrix, axis=1, keepdims=True) + 1e-8)  # Proporciones por fila
        richness = np.sum(data_matrix > 0, axis=1)  # Número de características presentes por fila
        diversity = -np.sum(relative_values * np.log(relative_values + 1e-8), axis=1)  # Diversidad tipo Shannon
        ...
        self.logger.info(f"Análisis de composición completado para {data_matrix.shape[0]} elementos y {data_matrix.shape[1]} características")
        return results
    ...
    def perform_dimensionality_reduction(
        ...
        """
        # Validar entradas
        self.validation_service.validate_input_shape(data_matrix, 2, "perform_dimensionality_reduction")
        ...
        if method == 'pca':
            ...
            # Calcular covarianza
            cov_matrix = BLASService.gemm(X_scaled.T, X_scaled) / (X_scaled.shape[0] - 1)
            ...
            try:
                ...
                self.logger.warning(f"Error en SVD personalizado, usando PCA de sklearn: {e}")
    ...
    def perform_clustering(
        ...
        """
        # Validar entradas
        self.validation_service.validate_input_shape(data_matrix, 2, "perform_clustering")
        ...
        if method == 'kmeans':
            ...
            calinski = calinski_harabasz_score(X_scaled, cluster_labels) if len(np.unique(cluster_labels)) > 1 else 0
        ...
        self.logger.info(f"Clustering completado ({method}): {silhouette:.3f} silhouette, {calinski:.3f} Calinski-Harabasz")
        return results
