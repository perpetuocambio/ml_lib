...
from ml_lib.visualization.services.visualization import VisualizationFactory, PlotConfig
...
from ml_lib.linalg.services.linalg import BLASService
...
class CustomEstimator(EstimatorInterface[np.ndarray, np.ndarray]):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, **kwargs) -> "CustomEstimator":
        ...
        self.is_fitted = True
        self.logger.info(
            f"Modelo ajustado: {X.shape[1]} características, bias={self.bias:.3f}"
        )
        return self
    ...
    def get_params(self) -> Dict[str, Any]:
        ...
        return {"weights": self.weights, "bias": self.bias, "is_fitted": self.is_fitted}
...
class DataTransformer(TransformerInterface[np.ndarray]):
    ...
    def inverse_transform(self, X: np.ndarray) -> np.ndarray:
        ...
        if not self.is_fitted:
            raise ValueError(
                "El transformador debe ser ajustado antes de realizar la transformación inversa"
            )
