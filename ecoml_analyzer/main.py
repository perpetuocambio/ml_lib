...
from ecoml_analyzer.data import AbundanceNormalizer, DataFilter
...
def generate_synthetic_ecological_data(
    n_species: int = 50, n_sites: int = 20, n_habitats: int = 3, random_state: int = 42
    ...
    for habitat in range(n_habitats):
        ...
        end_idx = (
            (habitat + 1) * species_per_habitat
            if habitat < n_habitats - 1
            else n_species
        )
        ...
        habitat_sites_end = (
            (habitat + 1) * sites_per_habitat if habitat < n_habitats - 1 else n_sites
        )
        ...
        for site_idx in range(habitat_sites_start, habitat_sites_end):
            for sp_idx in habitat_species:
                ...
                habitat_effect = (
                    10 if np.random.random() > 0.3 else 5
                )  # Mayor abundancia para especies de hábitat
...
def main():
    ...
    abundance_matrix, species_names, site_names, environmental_data = (
        generate_synthetic_ecological_data()
    )
    ...
    print(
        f"   - Datos normalizados usando transformador general: {normalized_matrix.shape}"
    )
    ...
    composition_results = analyzer.analyze_composition(
        filtered_matrix.T
    )  # Transponer para análisis por sitio
    ...
    print(
        f"   - Varianza explicada PC1: {dim_red_results['explained_variance_ratio'][0]:.2%}"
    )
    ...
    print(
        f"   - Calinski-Harabasz Score: {clustering_results['calinski_harabasz_score']:.1f}"
    )
    ...
    X_features = dim_red_results["transformed_data"][
        :, :3
    ]  # Usar primeros 3 componentes
    y_target = (filtered_matrix[0, :] > np.median(filtered_matrix[0, :])).astype(
        int
    )  # Clasificación basada en abundancia de primera especie
    ...
    print("   - Demostrando interfaz general EstimatorInterface aplicada a ecología")
    ...
    print("   - Aplicación de componentes generales de ml_lib al dominio ecológico")
    print("   - Uso de interfaces generales: TransformerInterface, EstimatorInterface")
    print(
        "   - Técnicas aplicadas: normalización, filtrado, PCA, clustering, clasificación"
    )
    print(f"   - Precisión del modelo ecológico: {accuracy:.3f}")

    print("\n✅ Demostración completada exitosamente!")
    print(
        "   La aplicación muestra cómo técnicas generales de ml_lib se aplican al dominio ecológico"
    )
    ...
    print("   - Interfaz TransformerInterface para transformadores de datos")
    print("   - Interfaz EstimatorInterface para modelos predictivos")
    print("   - Servicios de validación y logging")
    print("   - Operaciones de álgebra lineal general (SVD para PCA)")
    print("   - Aplicación agnóstica al dominio con enfoque ecológico")
...
def run_simple_example():
    ...
    print(
        "El ejemplo demuestra componentes generales de ml_lib aplicados a datos ecológicos"
    )
