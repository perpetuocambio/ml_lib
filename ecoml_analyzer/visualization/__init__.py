...
class EcologicalVisualizer:
    ...
    def plot_species_abundance_heatmap(
        ...
        if normalize:
            normalized_matrix = abundance_matrix / (
                np.sum(abundance_matrix, axis=0, keepdims=True) + 1e-8
            )
        else:
            normalized_matrix = abundance_matrix
        ...
        fig = self.general_viz.plot_heatmap(
            normalized_matrix, x_labels=site_names, y_labels=species_names, title=title
        )
    ...
    def plot_composition_analysis(
        ...
        composition_results: Dict[
            str, Any
        ],  # Usar Dict en lugar de np.ndarray para mayor flexibilidad
        labels: List[str],
        ...
        # Riqueza
        if "richness" in composition_results and isinstance(
            composition_results["richness"], np.ndarray
        ):
        ...
        # Diversidad
        if "diversity" in composition_results and isinstance(
            composition_results["diversity"], np.ndarray
        ):
            x_vals = np.arange(len(labels))
        ...
        # Abundancia total
        if "total_values" in composition_results and isinstance(
            composition_results["total_values"], np.ndarray
        ):
            x_vals = np.arange(len(labels))
    ...
    def plot_dimensionality_reduction(
        ...
        if X_transformed.shape[1] < 2:
            ...
            if "explained_variance_ratio" in dim_red_results:
                ...
                ax.set_ylabel(
                    f"{method}2 ({exp_var[1]:.2%} varianza)"
                    if len(exp_var) > 1
                    else f"{method}2"
                )
            else:
    ...
    def plot_clustering_results(
        ...
        scatter = ax.scatter(
            X_pca[:, 0], X_pca[:, 1], c=cluster_labels, cmap="tab10", alpha=0.7, s=60
        )
        ...
        ax.set_title(
            f'{title} (K={clustering_results["n_clusters"]}) - {clustering_results["clustering_method"]}'
        )
        ax.grid(True, alpha=0.3)
...
class InteractiveEcologicalVisualizer:
    ...
    def __init__(self):
        ...
        if not PLOTLY_AVAILABLE:
            self.logger.warning(
                "Plotly no está disponible. Instala 'plotly' para visualizaciones interactivas."
            )
    ...
    def plot_interactive_dimensionality_reduction(
        ...
        df = pd.DataFrame(
            {
                ...
                "Comp2": X_transformed[:, 1]
                if X_transformed.shape[1] > 1
                else np.zeros(len(X_transformed)),
            }
        )
        ...
        fig = px.scatter(
            df, x="Comp1", y="Comp2", title=title, hover_data=hover_data_cols
        )
        ...
        if "explained_variance_ratio" in dim_red_results:
            ...
            fig.update_layout(
                ...
                yaxis_title=f"{method}2 ({exp_var[1]:.2%} varianza)"
                if len(exp_var) > 1
                else f"{method}2",
                width=800,
                height=600,
