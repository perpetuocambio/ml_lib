...
class ErrorHandler:
    ...
    def handle_validation_error(self, func: Callable[P, R]) -> Callable[P, R]:
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
            try:
                ...
                self.logger.warning(f"Validation error in {func.__name__}: {str(e)}")
                raise
            except Exception as e:
                self.logger.error(
    ...
    def handle_resource_error(self, func: Callable[P, R]) -> Callable[P, R]:
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
            try:
                ...
                self.logger.critical(
                    f"Resource error in {func.__name__}: {str(e)}", exc_info=True
                )
                raise
            except Exception:
