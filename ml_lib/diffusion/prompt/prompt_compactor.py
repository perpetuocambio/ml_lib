...
class PromptCompactor:
    ...
    def _get_tokenizer(self) -> "CLIPTokenizer":  # type: ignore
        ...
        if self._tokenizer is None:
            try:
                ...
                logger.warning(
                    f"Failed to load CLIP tokenizer: {e}, using simple tokenizer"
                )
    ...
    def compact(
        ...
        if original_token_count <= self.max_tokens:
            logger.debug(
                f"Prompt already under limit ({original_token_count}/{self.max_tokens})"
            )
        ...
        for classification in classified_parts:
            ...
            if test_token_count <= self.max_tokens:
                ...
                if (
                    not kept_parts
                    and classification.priority == PromptTokenPriority.CRITICAL
                ):
        ...
        critical_parts = [
            p for p in classified_parts if p.priority == PromptTokenPriority.CRITICAL
        ]
        kept_critical = [
            p for p in kept_parts if p.priority == PromptTokenPriority.CRITICAL
        ]
