...
class LoRARecommender:
    ...
    def __init__(self, registry: ModelRegistry, config=None):
        ...
        self.POPULARITY_WEIGHT = config.scoring_weights.get(
            "popularity_score_weight", 0.10
        )
    ...
    def _filter_blocked_loras(self, loras: list[ModelMetadata]) -> list[ModelMetadata]:
        ...
        for lora in loras:
            ...
            if not is_blocked:
                ...
                logger.debug(
                    f"Blocked LoRA {lora.name}: contains anime/cartoon/underage tags"
                )
    ...
    def _priority_tag_score(self, lora: ModelMetadata) -> float:
        ...
        for priority_tag in self.PRIORITY_TAGS:
            if priority_tag in lora_text or any(
                priority_tag in tag for tag in lora_tags
            ):
                matches += 1
    ...
    def _generate_reasoning(
        ...
        if lora.trigger_words:
            ...
            if matched_triggers:
                reasons.append(
                    f"Trigger words found: {', '.join(matched_triggers[:2])}"
                )
