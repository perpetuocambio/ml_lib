...
class LearningEngine:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (
                    ...
                    feedback.rating,
                    json.dumps(feedback.user_modified_loras)
                    if feedback.user_modified_loras
                    else None,
                    json.dumps(feedback.user_modified_params)
            ...
            if feedback.user_modified_params:
                self._track_parameter_adjustments(cursor, feedback)

            conn.commit()

        logger.info(
            f"Recorded feedback {feedback.feedback_id} (rating: {feedback.rating})"
        )
    ...
    def get_insights(self) -> dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """)
            top_loras = [
                {"name": row[0], "avg_rating": round(row[1], 2), "count": row[2]}
    ...
    def clear_data(self, older_than_days: Optional[int] = None):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if older_than_days is None:
                ...
                cursor.execute(
                    "DELETE FROM parameter_adjustments WHERE timestamp < ?", (cutoff,)
                )
