...
from ml_lib.diffusion.generation.memory_optimizer import MemoryMonitor
from ml_lib.diffusion.generation.model_orchestrator import (
    ModelOrchestrator,
    DiffusionArchitecture,
)
from ml_lib.diffusion.registry.model_registry import ModelRegistry
from ml_lib.diffusion.prompt.ollama_selector import (
    OllamaModelSelector,
    ModelMatcher,
)
...
from ml_lib.diffusion.prompt.content_tags import PromptCompactionResult
from ml_lib.diffusion.generation.user_preferences_db import (
    UserPreferencesDB,
    UserPreferences,
)
...
class GenerationConfig:
    ...
    # Core
    prompt: str
    negative_prompt: str = "worst quality, low quality, blurry, distorted, bad anatomy"
...
class IntelligentPipelineBuilder:
    ...
    def generate(
        ...
        **kwargs,
        ...
        config = GenerationConfig(
            prompt=prompt,
            negative_prompt=negative_prompt or "worst quality, low quality, blurry",
            style=style,
            quality=quality,
    ...
    def _select_models(self, config: GenerationConfig) -> SelectedModels:
        ...
        if self.enable_ollama:
            try:
                ...
                if selector.stop_server(force=True):
                    logger.info(
                        "Stopped Ollama server to free GPU memory for generation"
                    )
                    ...
                    logger.warning(
                        "Failed to stop Ollama server, generation may use more memory"
                    )
        ...
        if not selected_base and base_models:
            ...
            if config.style:
                for model in sorted(
                    base_models, key=lambda m: m.popularity_score, reverse=True
                ):
        ...
        if selected_base:
            ...
            logger.info(
                f"Selected base model: {selected_base.model_name} ({base_architecture.value})"
            )
        else:
        ...
        if vaes and selected_base:
            ...
            for vae in sorted(vaes, key=lambda v: v.popularity_score, reverse=True):
                ...
                if any(
                    pattern in vae_name_lower
                    for pattern in arch_info.compatible_vae_patterns
                ):
        ...
        opt_level = self._determine_optimization_level(
            ...
            available_memory_gb=resources.available_gpu_memory_gb(),
        )
        ...
        if self.user_prefs_db and self.user_id:
            base_model_path, selected_loras, lora_weights, steps, cfg_scale, sampler = (
    ...
    def _apply_user_preferences(
        ...
        if base_model_name in prefs.blocked_models:
            logger.warning(
                f"Base model '{base_model_name}' is blocked by user preferences, finding alternative..."
            )

            # Try to find alternative from favorites
            if prefs.favorite_base_models:
                # Get all available base models
                base_models = self.orchestrator.metadata_index.get(
                ...
                for fav_name in prefs.favorite_base_models:
                    for model in base_models:
                        if (
                            fav_name.lower() in model.model_name.lower()
                            or fav_name.lower() in model.file_name.lower()
                        ):
                            base_model_path = model.file_path
                            logger.info(
                                f"Using favorite model instead: {model.model_name}"
                            )
                            break
                    else:
                        continue
    ...
    def _load_pipeline(self, selected: SelectedModels):
        ...
        try:
            ...
            if architecture in [BaseModel.SD15, BaseModel.SD20, BaseModel.SD21]:
                ...
                logger.warning(
                    f"Unknown architecture {architecture}, using auto pipeline"
                )
                pipeline = AutoPipelineForText2Image.from_single_file(
                    str(selected.base_model_path),
                    torch_dtype=torch.float16
            ...
            if selected.lora_paths:
                for lora_path, weight in zip(
                    selected.lora_paths, selected.lora_weights
                ):
                    if lora_path.exists():
                        try:
                            ...
                            if hasattr(pipeline, "set_adapters"):
                                pipeline.set_adapters(
                                    [lora_path.stem], adapter_weights=[weight]
                                )

                            logger.info(
                                f"Loaded LoRA: {lora_path.name} (weight: {weight:.2f})"
                            )
                        except Exception as e:
    ...
    def _configure_scheduler(self, pipeline, sampler: str, scheduler_type: str):
        ...
        try:
            ...
            logger.warning(
                f"Failed to configure scheduler {sampler}: {e}, using default"
            )
    ...
    def _generate_images(
        ...
        try:
            ...
            optimized_positive, optimized_negative = (
                self.prompt_analyzer.optimize_for_model(
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "resources": {
                "gpu_memory_gb": self.resource_monitor.get_current_stats().available_gpu_memory_gb(),
