...
class ModelOrchestrator:
        ...
        for model_type in ModelType:
            try:
                ...
                if db_models:
                    ...
                    logger.debug(
                        f"Loaded {len(db_models)} {model_type.value} models from database"
                    )
    ...
    def _auto_populate_database(self) -> None:
        ...
        try:
            ...
            from ml_lib.diffusion.sources.comfyui_migrator import (
                ComfyUIMetadataMigrator,
            )
            ...
            logger.info(
                f"Auto-population complete: {total_success} models added, {total_failed} failed"
            )
    ...
    def _convert_metadata_to_file(self, metadata) -> ModelMetadataFile:
        ...
        return ModelMetadataFile(
            file_name=metadata.local_path.name
    ...
    def select_compatible_loras(
        ...
        compatible = [
            lora for lora in loras if lora.get_base_model_enum() == base_model
        ]
