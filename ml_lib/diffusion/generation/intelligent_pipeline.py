...
from ml_lib.diffusion.generation.parameter_optimizer import ParameterOptimizer
from ml_lib.diffusion.generation.learning_engine import LearningEngine
from ml_lib.diffusion.lora.lora_recommender import LoRARecommender
from ml_lib.diffusion.generation.model_offloader import ModelOffloader
from ml_lib.system.services.memory_manager import MemoryManager
from ml_lib.diffusion.generation.memory_optimizer import (
    MemoryOptimizer,
    MemoryOptimizationConfig,
    OptimizationLevel,
    MemoryMonitor,
)
from ml_lib.diffusion.generation.learning_engine import GenerationFeedback
...
class IntelligentGenerationPipeline:
    ...
    def _get_optimization_level(self) -> OptimizationLevel:
        ...
        vram = (
            self.memory_manager.resources.available_vram_gb
    ...
    def generate(
        ...
        **overrides: str,
        ...
        peak_vram = (
            self.memory_manager.get_peak_vram_usage() if self.memory_manager else 0.0
        )
        ...
        metadata = GenerationMetadata(
            ...
            loras_used=[
                LoRAInfo(name=r.lora_name, alpha=r.suggested_alpha) for r in lora_recs
            ],
    ...
    def _load_base_model(self, model_id: str):
        ...
        dtype = (
            torch.float16
            if self.config.memory_settings.enable_quantization
            else torch.float32
        )
        ...
        if self.memory_optimizer:
            logger.info(
                "Applying EXTREME memory optimizations (market differentiator)..."
            )
            ...
            try:
                ...
                if lora_path:
                    self.diffusion_pipeline.load_lora_weights(
                        lora_path, adapter_name=rec.lora_name
                    )
                    # Set LoRA scale
                    self.diffusion_pipeline.set_adapters(
                        [rec.lora_name], adapter_weights=[rec.suggested_alpha]
                    )
    ...
    def _generate_image(
        ...
        try:
            ...
            if self.memory_optimizer:
                ...
                logger.info(
                    f"Peak memory during generation: {monitor.get_peak_memory():.2f}GB"
                )
            else:
                result = self.diffusion_pipeline(
                    prompt=prompt,
    ...
    def _apply_learning_adjustments(
        ...
        for rec in lora_recs:
            adjustment = self.learning_engine.get_lora_adjustment_factor(rec.lora_name)
            if adjustment != 1.0:
