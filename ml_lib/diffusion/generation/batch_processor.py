...
class BatchProcessor:
    ...
    def _param_variation(
        ...
        param_variations = self._generate_param_variations(base_recs, config.num_images)
    ...
    def _lora_variation(
        ...
        for i in range(config.num_images):
            logger.debug(
                f"Generating image {i + 1}/{config.num_images} (LoRA variation)"
            )
    ...
    def _mixed_variation(
        ...
        for i in range(config.num_images):
            if i % 2 == 0 and param_variations:
                # Use param variation
                params = (
                    param_variations[i // 2] if i // 2 < len(param_variations) else {}
                )
                result = self.pipeline.generate(
