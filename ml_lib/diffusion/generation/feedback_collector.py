...
class GenerationSession:
    ...
    modifications: ParameterModifications = field(
        default_factory=lambda: ParameterModifications()
    )
...
class FeedbackCollector:
    ...
    def start_session(
        ...
        recommended_loras = [lora.lora_name for lora in recommendations.suggested_loras]
        recommended_params = GenerationParameters(
        ...
        actual_params_final = (
            actual_params if actual_params is not None else recommended_params
        )
        ...
        if user_modified:
            ...
            if actual_params_final.seed != recommended_params.seed:
                modifications_dict["seed"] = ParameterModification(
                    recommended_value=recommended_params.seed
                    if recommended_params.seed is not None
                    else "None",
                    actual_value=actual_params_final.seed
                    if actual_params_final.seed is not None
                    else "None",
                )
        ...
        logger.debug(
            f"Started session {generation_id[:8]}... (modified: {user_modified})"
        )
    ...
    def _send_to_learning_engine(
        ...
        if session.actual_loras != session.recommended_loras:
            user_modified_loras = {
                ...
                "added": [
                    lora
                    for lora in session.actual_loras
                    if lora not in session.recommended_loras
                ],
                "removed": [
                    lora
                    for lora in session.recommended_loras
                    if lora not in session.actual_loras
                ],
            }
    ...
    def get_feedback_stats(self) -> FeedbackStatistics:
        ...
        liked_count = sum(1 for f in self._feedback_history if f.liked is True)
    ...
    def export_feedback_data(self, output_path: Path) -> None:
        ...
        data = {
            ...
            "common_tags": [{"tag": tc.tag, "count": tc.count} for tc in common_tags],
            "feedback_history": [
                {
