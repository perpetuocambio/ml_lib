...
class IPAdapterService:
    ...
    def load_clip_vision(
        ...
        """
        try:
            from ml_lib.diffusion.models.clip.clip_vision_handler import (
                load_clip_vision,
            )
    ...
    def extract_features_batch(
        ...
        if self.clip_vision_encoder is not None:
            ...
            return [self.extract_features(img, return_patch_features) for img in images]
    ...
    def apply_conditioning(
        ...
        try:
            ...
            if model_info.model:
                ...
                # Try HuggingFace Hub
                logger.info(
                    f"Loading IP-Adapter from HuggingFace: {model_info.config.model_id}"
                )
                pipeline.load_ip_adapter(
                    "h94/IP-Adapter",
                    subfolder="models",
