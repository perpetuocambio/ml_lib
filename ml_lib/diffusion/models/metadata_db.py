        ...
        try:
            with self.connection() as conn:
                ...
                for word in metadata.trigger_words:
                    cursor.execute(
                        "INSERT OR IGNORE INTO trigger_words (word) VALUES (?)", (word,)
                    )
                    cursor.execute(
                        "SELECT id FROM trigger_words WHERE word = ?", (word,)
                    )
    ...
    def get_model(self, model_id: str):
        ...
        try:
            with self.connection() as conn:
                ...
                from ml_lib.diffusion.models.model_metadata import ModelMetadata
    ...
    def search_models(
        ...
        try:
            with self.connection() as conn:
                ...
                query += (
                    " ORDER BY m.rating DESC, m.download_count DESC LIMIT ? OFFSET ?"
                )
    ...
    def get_stats(self) -> dict:
        ...
        try:
            with self.connection() as conn:
                ...
                # Count by type
                cursor.execute(
                    "SELECT type, COUNT(*) as count FROM models GROUP BY type"
                )
                stats["by_type"] = {
                ...
                # Count by base model
                cursor.execute(
                    "SELECT base_model, COUNT(*) as count FROM models GROUP BY base_model"
                )
                stats["by_base_model"] = {
                    row["base_model"]: row["count"] for row in cursor.fetchall()
                }
                ...
                cursor.execute(
                    "SELECT COUNT(*) as local FROM models WHERE local_path IS NOT NULL"
                )
    ...
    def get_user_override(self, model_id: str) -> Optional[dict]:
        ...
        try:
            with self.connection() as conn:
                ...
                cursor.execute(
                    "SELECT override_data FROM user_overrides WHERE model_id = ?",
                    (model_id,),
                )
                row = cursor.fetchone()
    ...
    def delete_user_override(self, model_id: str) -> bool:
        ...
        try:
            with self.connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "DELETE FROM user_overrides WHERE model_id = ?", (model_id,)
                )
                conn.commit()
                return True
        except Exception as e:
