...
from ml_lib.diffusion.models.common.model_enums import ModelType, BaseModel, Source
...
class MetadataScraper:
    ...
    def sync_local_models(
        self, models_dir: Path, model_type: ModelType
    ) -> dict[str, int]:
        """
        ...
        logger.info(
            f"Local sync complete: {results['added']} added, {results['skipped']} skipped"
        )
        return results
    ...
    def _get_by_local_path(self, local_path: Path):
        ...
        try:
            rows = self.db.execute(
                "SELECT model_id FROM models WHERE local_path = ?", (str(local_path),)
            )
            if rows:
    ...
    def _create_local_metadata(
        ...
        from ml_lib.diffusion.models.common.model_enums import ModelFormat

        model_format = ModelFormat(
            format_map.get(model_file.suffix.lower(), "safetensors")
        )
