...
class VisualizationErrorHandler:
    ...
    def handle_visualization_error(self, func: Callable[P, R]) -> Callable[P, R]:
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
            try:
                ...
                self.logger.error(f"Error de dependencias en {func.__name__}: {str(e)}")
                raise
...
class VisualizationConfigHandler:
    ...
    def validate_data_shape(data: np.ndarray, expected_dims: int = None) -> bool:
        ...
        if expected_dims and data.ndim != expected_dims:
            raise ValueError(
                f"Se esperaban {expected_dims} dimensiones, se recibieron {data.ndim}"
            )
        return True
