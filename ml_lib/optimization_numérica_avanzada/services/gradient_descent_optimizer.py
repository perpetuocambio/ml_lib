...
from ..models.optimization_models import GradientDescentConfig
...
class GradientDescentOptimizer(Optimizer):
        ...
        for iteration in range(self.config.maxiter):
            try:
                ...
                if self.config.verbose and iteration % 10 == 0:
                    print(f"Iteración {iteration}: f(x) = {f_new:.6e}, ||grad|| = {np.linalg.norm(grad):.6e}")
            ...
            message = self.error_handler.handle_convergence_failure(self.config.maxiter, iteration)
    ...
    def _adam_update(self, x: np.ndarray, grad: np.ndarray, m: np.ndarray, 
                     v: np.ndarray, t: int) -> tuple:
        """
    ...
    def _momentum_update(self, x: np.ndarray, grad: np.ndarray, 
    ...
    def _adagrad_update(self, x: np.ndarray, grad: np.ndarray, 
                        g_squared: np.ndarray) -> tuple:
        """
        Actualización Adagrad para descenso de gradiente.
        """
        g_squared += grad ** 2
        x_new = x - self.config.learning_rate * grad / (np.sqrt(g_squared) + self.config.eps)
        return x_new, g_squared
    
    def _rmsprop_update(self, x: np.ndarray, grad: np.ndarray, 
                        g_squared: np.ndarray) -> tuple:
        """
