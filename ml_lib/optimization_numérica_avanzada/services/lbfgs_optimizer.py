...
from ..models.optimization_models import LBFGSConfig
...
class LBFGSOptimizer(Optimizer):
        ...
        for iteration in range(self.config.maxiter):
            try:
                ...
                if self.config.verbose and iteration % 10 == 0:
                    print(f"Iteración {iteration}: f(x) = {f_new:.6e}, ||grad|| = {np.linalg.norm(grad_new):.6e}")
    ...
    def _two_loop_recursion(self, grad: np.ndarray, s_list: list, 
                           y_list: list, rho_list: list) -> np.ndarray:
        """
    ...
    def _line_search(self, func: Callable, x: np.ndarray, p: np.ndarray, 
                     grad: np.ndarray, jac: Optional[Callable] = None,
