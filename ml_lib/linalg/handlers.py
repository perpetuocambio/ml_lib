...
# Importar modelos
from .models import Matrix, MatrixOperationConfig
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
            try:
                ...
                self.logger.critical(
                    f"Error de memoria en {func.__name__}: {str(e)}"
                )
                raise
...
class MatrixConfigHandler:
    ...
    def validate_matrix_shape(matrix: np.ndarray, expected_dims: int = None) -> bool:
        ...
        if expected_dims and matrix.ndim != expected_dims:
            raise ValueError(f"Se esperaban {expected_dims} dimensiones, se recibieron {matrix.ndim}")
        return True
    ...
    def sanitize_matrix_config(config: MatrixOperationConfig) -> MatrixOperationConfig:
        ...
        config.memory_alignment = max(8, min(256, config.memory_alignment))  # Alineación
...
class MatrixValidationHandler:
    ...
    def validate_square_matrix(matrix: np.ndarray) -> None:
        ...
        if matrix.ndim != 2 or matrix.shape[0] != matrix.shape[1]:
            raise ValueError(f"La matriz debe ser cuadrada, pero es de forma {matrix.shape}")
    ...
    def validate_positive_definite(matrix: np.ndarray) -> None:
        ...
        try:
            ...
            if np.any(eigenvalues <= 0):
                raise ValueError("La matriz no es definida positiva (tiene valores propios no positivos)")
        except np.linalg.LinAlgError:
            raise ValueError("No se pudieron calcular los valores propios para verificar definida positiva")
    ...
    def validate_invertible(matrix: np.ndarray, rtol: float = 1e-10) -> None:
        ...
        try:
            ...
            if abs(det) < rtol:
                raise ValueError(f"La matriz no es invertible (determinante ≈ {det:.2e})")
        except np.linalg.LinAlgError:
            raise ValueError("No se pudo calcular el determinante para verificar invertibilidad")