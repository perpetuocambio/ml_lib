...
from .interfaces import MatrixOperationInterface, DecompositionInterface, SolverInterface, BLASInterface, LAPACKInterface
...
class BLASService(BLASInterface[np.ndarray]):
    ...
    def gemm(
        ...
        if k_A != k_B:
            raise ValueError(f"Dimensiones incompatibles: A es {A.shape}, B es {B.shape}")
        
        # Crear matriz C si no se proporciona
        if C is None:
            C = np.zeros((m, n), dtype=np.result_type(A.dtype, B.dtype))
        elif C.shape != (m, n):
            raise ValueError(f"Forma de C incorrecta: esperada {(m, n)}, obtenida {C.shape}")
    ...
    def gemv(
        ...
        m, n = A.shape
        if len(x) != n:
            raise ValueError(f"Dimensiones incompatibles: A es {A.shape}, x es {x.shape}")
        
        # Crear vector y si no se proporciona
        if y is None:
            y = np.zeros(m, dtype=np.result_type(A.dtype, x.dtype))
        elif len(y) != m:
            raise ValueError(f"Forma de y incorrecta: esperada {(m,)}, obtenida {y.shape}")
...
class SparseMatrixService:
    ...
    def sparse_matmul(A: SparseMatrix, B: np.ndarray) -> np.ndarray:
        ...
        if A.format == "COO":
            sparse_A = coo_matrix((A.data, (A.row_indices, A.col_indices)), shape=A.shape)
        elif A.format == "CSR":
