...
class GPUStats:
    ...
    def is_thermal_throttling(self, threshold: float = 80.0) -> bool:
        ...
        return self.temperature_celsius is not None and self.temperature_celsius >= threshold
...
class ResourceMonitor:
    ...
    def get_recommended_device(self, model_size_gb: float) -> str:
        ...
        if self.can_fit_model(model_size_gb, "cuda"):
            ...
            logger.warning(
                f"Model ({model_size_gb}GB) may not fit in available memory"
            )
            return "cpu"  # Fallback
    ...
    def print_summary(self):
        ...
        if stats.has_gpu():
            for gpu in stats.gpus:
                ...
                print(f"   Memory: {gpu.memory_used_gb:.1f}GB / {gpu.memory_total_gb:.1f}GB ({gpu.memory_percent:.1f}%)")
